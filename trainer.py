    ...
    def create_optimizer(self):
        ...
        if not self.protein_model_fixed:
            for n, p in self.model.named_parameters():
                if "structure_bias" in n:
                    ratio_parameters.append(n)
                if "protein_projection" in n:
                    ratio_parameters.append(n)
                ...
                if "mlm_head" in n:
                    ratio_parameters.append(n)
            print(f"Ratio parameters: {ratio_parameters}")
...
class PITPretrainTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.protein_model_fixed and not isinstance(
            self.model.protein_model, PeftModel
        ):
            for param in self.model.protein_model.parameters():
                param.requires_grad = False
        if self.text_model_fixed and not isinstance(
            self.model.protein_model, PeftModel
        ):
            for param in self.model.text_model.parameters():
                param.requires_grad = False
        ...
        if not self.protein_model_fixed:
            ratio_parameters += [
                n for n, p in self.model.protein_model.named_parameters()
            ]
